  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-muted-foreground">Services installés</h2>
        ...
        <div className="min-w-0">
          ...
          {service.installed ? (
            <p className="mt-1 flex items-center gap-1 text-sm text-primary">
              ...
              {service.running ? "En cours d'exécution" : "Installé, arrêté"}
              {service.version ? ` · ${service.version}` : ""}
            </p>
          ) : (
            <p className="mt-1 flex items-center gap-1 text-sm text-muted-foreground">
              <XCircle className="h-4 w-4" /> Non installé
            </p>
          )}
          {service.detail && (
            ...
            <ConfirmDialog
              trigger={
                <Button size="sm" variant="outline" disabled={busyAction === "tailscale-update"}>
                  {busyAction === "tailscale-update" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Mettre à jour"}
                </Button>
              }
              title="Mettre à jour Tailscale ?"
              description="Lance tailscale update sur le serveur."
              ...
              <ConfirmDialog
                trigger={
                  <Button size="sm" variant="outline" disabled={busyAction === "docker-update"}>
                    {busyAction === "docker-update" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Mettre à jour"}
                  </Button>
                }
                title="Mettre à jour Docker ?"
                description="Met à jour docker-ce, docker-ce-cli et containerd.io via apt."
                confirmLabel="Mettre à jour"
              ...
              <ConfirmDialog
                trigger={
                  <Button size="sm" variant="destructive" disabled={busyAction === "docker-restart"}>
                    {busyAction === "docker-restart" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Redémarrer"}
                  </Button>
                }
                title="Redémarrer Docker ?"
                description="Tous les conteneurs seront brièvement arrêtés puis relancés (selon leur politique de redémarrage). Action disruptive."
            ...
            <ConfirmDialog
              trigger={
                <Button size="sm" variant="outline" disabled={busyAction === "pm2-reload"}>
                  {busyAction === "pm2-reload" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Recharger le daemon"}
                </Button>
              }
              title="Recharger le daemon PM2 ?"
              description="Lance pm2 update pour recharger le daemon en mémoire."
      ...
      {result && (
        <details className="mt-3 text-xs">
          <summary className={result.ok ? "cursor-pointer text-primary" : "cursor-pointer text-destructive"}>
            {result.ok ? "Résultat : succès" : "Résultat : échec"}
          </summary>

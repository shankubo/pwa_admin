...
Le service `pwa-admin` tourne directement sur le serveur (pas containerisé), écoutant en HTTPS natif sur le port 8443. Sur les 2 serveurs de production existants (Pi, `ubuntu_ext`/fr01pc999), il tourne sous l'utilisateur admin `shan` (pas de compte système dédié — choix historique, serveur mono-admin). Pour une **nouvelle** installation, `deploy/install.sh --create-user` crée et utilise à la place un compte système dédié (`nologin`, UID système) — voir README.md section "Compte dédié". Les 2 serveurs existants ne sont pas migrés. `apps/api` sert à la fois l'API `/api/*` et les fichiers statiques buildés de `apps/web` (`fastify-static`, fallback SPA vers `index.html`).
...
- **Exception explicite et unique à cette règle** : `deploy/auto-update.sh` (polling cron toutes les 5 min sur le RPi 192.168.1.88 uniquement, pas sur `ubuntu_ext`/fr01pc999) compare `HEAD` à `origin/master`, et si en retard, fait `git pull --ff-only` + rebuild (shared→api→web) + `sudo systemctl restart pwa-admin` de façon autonome. C'est un choix délibéré (pas de webhook GitHub possible : l'app est Tailscale-only, GitHub ne peut pas atteindre une IP Tailscale privée, donc polling local plutôt qu'un tunnel public). Ceci nécessite la règle sudoers `shan ALL=(root) NOPASSWD: /bin/systemctl restart pwa-admin` dans `deploy/sudoers.d/pwa-admin` — **ne jamais élargir cette règle** à d'autres commandes ou contextes ; c'est la seule voie automatisée approuvée pour ce self-restart. Logs dans `data/auto-update.log`. Cron installé manuellement côté serveur (`crontab -e`), pas versionné dans le repo.
...
- `deploy/sudoers.d/pwa-admin` (fichier littéral `shan`) et `deploy/sudoers.d/pwa-admin.template` (tokens `__PWA_ADMIN_USER__`/`__PWA_ADMIN_APP_DIR__`, substitué automatiquement par `install.sh --create-user`) doivent rester en phase : toute règle ajoutée/modifiée dans l'un doit être répercutée dans l'autre. Le premier est ce que les 2 serveurs existants recopient manuellement ; le second est la source de vérité pour les nouvelles installations à compte dédié.
